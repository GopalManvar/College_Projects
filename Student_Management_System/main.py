...
# This are the all modules requred imported
...
# this is for getting unique user access without login we can't further procied
...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/student_System'
db=SQLAlchemy(app)
...
#Student Adding information format for the database
...
# This command used to test the database connection established or not  
@app.route('/test')
def test():
    try:
        Test.query.all()
